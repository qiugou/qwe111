...
export default {
  ...
  methods: {
        ...
        result.then(r => {
          ...
          try {
            if ((['auto', true].includes(this.showPagination) &&
              r.total <= (r.pageNum * this.localPagination.pageSize))) {
    ...
    renderAlert () {
      ...
      if (typeof this.alert === 'boolean') {
        ...
        // 绘制 清空 按钮
        clearItem = (typeof this.alert.clear === 'boolean' && this.alert.clear) ? (
  ...
  render () {
    ...
    const showAlert = (typeof this.alert === 'object' && this.alert !== null && this.alert.show) &&
      this.rowSelection && typeof this.rowSelection.selectedRowKeys !== 'undefined' || this.alert
